"""
Created on Wed Jun  2 21:51:01 2021
...
seq_length = 5 # INput length of the sequence
...
N_v = 200 # the number of masked nodes for training
...
cmems_data = loadmat('modis/modis_rescale_unknow0.mat')

X = cmems_data['X']
dist_mx = cmems_data['dist_mx']
...
Define model
'''
modis_kriging = general_satcn(avg_d, device, layers = layers, t_kernel = t_kernel) # very simple model, without tuning feature number aggregation, scaler
modis_kriging.to(device)
optimizer = optim.Adam(modis_kriging.parameters(), lr=0.001, weight_decay=0.0001)
...
for i in range(1, episode+1):
    ...
    for t in range((training_set.shape[1]  -  (layers + 1) *  (t_kernel - 1) - tseq_length)//(tseq_length * batch_size)):
        ...
        modis_kriging.train()
        optimizer.zero_grad()
        pre = modis_kriging(trainx, Atra2, Atra1) 
        
        loss = masked_mae(pre, trainy, 0.0)
        loss.backward()
        torch.nn.utils.clip_grad_norm_(modis_kriging.parameters(), 2)
        optimizer.step()
    ...
    for t in range((test_set.shape[1] - (layers + 1) *  (t_kernel - 1) - tseq_length)//tseq_length):
        ...
        modis_kriging.eval()
        with torch.no_grad():
            pred = modis_kriging(testx, Ate2, Ate1)
    ...
    if test_mae == min(MAE_list):
        ...
        best_model = copy.deepcopy(modis_kriging.state_dict())

modis_kriging.load_state_dict(best_model)
torch.save(modis_kriging, 'modis' + str(training_rate) + str(sample_rate) + str(layers) + str(least_k) + str(missing_rate) + '.pth')
